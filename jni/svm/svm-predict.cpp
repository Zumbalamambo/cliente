...
int predict(float **values, int **indices, int rowNum, int colNum, int *labels, double *prob_estimates, int isProb)
{
	...
	if(isProb)
	{
		if (svm_type==NU_SVR || svm_type==EPSILON_SVR)
			LOGD(LOG_TAG, "Prob. model for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma=%g\n",svm_get_svr_probability(model));
		else
		{
        ...
        int isProb, const char *modelFile, int *labels, double* prob_estimates)
{
	...
	{
		LOGD(LOG_TAG, "can't open model file %s\n",modelFile);
		exit(1);
	}
	...
	if(isProb)
	{
		...
		{
			LOGD(LOG_TAG, "Model does not support probabiliy estimates\n");
			exit(1);
		}
	}
	else
	{
		if(svm_check_probability_model(model)!=0)
			LOGD(LOG_TAG, "Model supports probability estimates, but disabled in prediction.\n");
	}
