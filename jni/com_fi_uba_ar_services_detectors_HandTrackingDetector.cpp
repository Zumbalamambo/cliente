...
static jint trainClassifier(JNIEnv *env, jobject obj, jstring trainingFileS,
		jint kernelType, jint cost, jfloat gamma, jint isProb,
		jstring modelFileS) {
	jboolean isCopy;
...
static JNINativeMethod sMethods[] =
		...
		{ "doClassificationNative", "([[F[[IILjava/lang/String;[I[D)I",
				(void*) doClassification }, { "trainClassifierNative",
				"(Ljava/lang/String;IIFILjava/lang/String;)I",
...
static int jniRegisterNativeMethods(JNIEnv *env, const char *className,
	...
	if (clazz == NULL) {
		LOGE("jniRegisterNativeMethods", "Native registration unable to find class '%s'", className);
		return JNI_FALSE;
	}

	if (env->RegisterNatives(clazz, Methods, numMethods) < 0) {
		LOGE("jniRegisterNativeMethods", "RegisterNatives failed for '%s'", className);
		return JNI_FALSE;
	}
	return JNI_TRUE;
}
...
int register_Signal(JNIEnv *env) {
	return jniRegisterNativeMethods(env,
			"com/fi/uba/ar/services/detectors/HandTrackingDetector", sMethods,
