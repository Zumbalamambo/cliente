package com.fi.uba.ar.detectors;
...
public class MarkerDetector {
	...
	public void detect(Mat in, Vector<Marker> detectedMarkers, //CameraParameters cp, Mat camMatrix, Mat distCoeff,
		...
		CustomLog.d(TAG, "Mat in = " + in);
		CustomLog.d(TAG, "Mat is type CvType.CV_8UC1 = " + (in.type() == CvType.CV_8UC1));
		// do the threshold of image and detect contours
//		Imgproc.cvtColor(in, grey, Imgproc.COLOR_BGR5652GRAY);		
//		Imgproc.cvtColor(in, grey, Imgproc.COLOR_RGBA2GRAY);
		grey = in; // ya enviamos la mat con una imagen en greyscale
		...
		CustomLog.d(TAG, "contours2.size() = " + contours2.size());
...
//							candidateMarkers.lastElement().add(new Point(points[6],points[7]));
		...
		CustomLog.d(TAG, "nCandidates = " + nCandidates);
		...
		for(int i=0;i<nCandidates;i++){
			if(toRemove.get(i) == 0){
				...
				//XXX: esto hace una transformacion con perspectiva.. pero despues no lee el codigo QR asi que no se
				...
				if (in.width() >= in.height())
					...
					mat_size = in.height();
				warp(in, canonicalMarker, new Size(mat_size,mat_size), marker.toList());
//				List<Point> points = marker.toList();
				
				// obtenemos submat del rectangulo definido por los puntos del marker
//				Rect r = new Rect(points.get(0), points.get(1));
//				CustomLog.d(TAG, "rect = " + r);
//				in.submat(r).copyTo(canonicalMarker);
				//XXX: enviamos un evento para que el Debug View muestre el canonicalMarker que es un posible marker detectado
//				EventBus.getDefault().post(new DisplayImageDebugEvent(MatUtils.matToBitmap(canonicalMarker)));
				
				marker.setMat(canonicalMarker);
				//marker.extractCode();  //XXX: no es necesario
				if(marker.checkBorder()) {
					//XXX: no extraemos el id como aruco sino que simplemente si encontramos el borde asumimos que es un posible marker
					// y verificamos si hay codigo QR
//					int id = marker.calculateMarkerId();
//					if(id != -1){
//					if (marker.extractQRCode().size() > 0) { // si se encuentra un codigo QR entonces es un marker valido
						marker.setMarkerId(i); // ponemos un id de acuerdo a lo que encontro
