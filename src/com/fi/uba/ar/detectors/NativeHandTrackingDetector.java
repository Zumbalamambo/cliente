...
public class NativeHandTrackingDetector {
	...
	public static Mat detect(Mat mat) {
		...
		//TODO: debemos agregar aca algun control de que esto se haga solo si esta habilitada
		// en la aplicacion la deteccion de manos
		// Si esto se desactiva, automaticamente se debe desactivar la deteccion de gestos tambien
		
		if (!MainApplication.getInstance().isHandDetectionEnabled())
			return null;
		...
		if (result != null) {
			if (result.length > 0) {				
				...
				// si no esta habilitado el uso del debug fragment/view esto podria devolver null
				if (hdf != null) {
					//CustomLog.d(TAG, "HandDebugFragment no es null");
					hdf.addFingerTipPoints(points);
				}
				
				//Mandamos todos los dedos detectados al HandGestureController 
				// para que se puedan generar los eventos de touch necesarios
